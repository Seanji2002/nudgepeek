...
contextBridge.exposeInMainWorld('nudgeHistory', {
  getStoredSession: () =>
    ipcRenderer.invoke(CHANNELS.GET_SESSION),

  updateSession: (session: unknown) =>
    ipcRenderer.send(CHANNELS.SESSION_UPDATE, session),

  openImageDialog: () =>
    ipcRenderer.invoke(CHANNELS.OPEN_IMAGE),

  sendIncomingPhoto: (payload: unknown) =>
    ipcRenderer.send(CHANNELS.PHOTO_INCOMING, payload),
  ...
  getAutoLaunchStatus: () =>
    ipcRenderer.invoke(CHANNELS.AUTOLAUNCH_GET),

  setAutoLaunch: (enabled: boolean) =>
    ipcRenderer.send(CHANNELS.AUTOLAUNCH_SET, enabled),
})
