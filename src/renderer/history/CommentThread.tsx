...
export default function CommentThread({ photoId, userId }: Props) {
  ...
  useEffect(() => {
    return commentBus.subscribe((event) => {
      if (event.kind === 'insert') {
        if (event.comment.photoId !== photoId) return
        setComments((prev) =>
        ...
        if (event.photoId !== photoId) return
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      const real = await postComment(photoId, userId, body)
      setComments((prev) => {
