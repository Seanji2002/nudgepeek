...
export default function CameraCapture({ userId, onClose, onSendStart, onSendEnd, onSendError, onFallbackToFile }: Props) {
  ...
  useEffect(() => {
    ...
    async function startCamera() {
      try {
        ...
        if (name === 'NotAllowedError' || msg.toLowerCase().includes('permission') || msg.toLowerCase().includes('notallowed')) {
          setCamError('Camera permission denied. Check your browser/OS settings and try again.')
        } else if (name === 'NotFoundError' || name === 'DevicesNotFoundError' || msg.toLowerCase().includes('notfound') || msg.toLowerCase().includes('not found')) {
          setCamError('No camera found. If you are on WSL2, cameras are not available — use "Pick from file" below instead.')
